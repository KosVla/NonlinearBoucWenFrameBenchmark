function [MODEL] = InputFileLinksFirst_Localize(HistDamaged)
...
%The first term should indicate the hysteretic link, the next 8 terms the
...
nl_links_alternate=[7 8 10 11 20 21 24 25];
MODEL.nl_links_alternate =zeros(length(nl_links_alternate),9);
MODEL.nl_links_alternate(:,1)=nl_links_alternate;
MODEL.nl_links_alternate(:,2:end)=repmat(...
    [bw_a bw_k Alpha Beta Gamma N deltav deltan],length(nl_links_alternate),1);
