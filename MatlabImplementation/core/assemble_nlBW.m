...
Properties=MODEL.BW.HystLinks; HIST=MODEL.BW.HistBW;

Nodes=Properties(:,1:2);
link_properties = Properties(:,3:end);

switch MODEL.BW.integration_method
    case 'Euler'
        [ Rs,Ks, HIST ] = bw_Euler( MODEL.u, Nodes, HIST, link_properties);
    case 'RK2'
        [ Rs,Ks, HIST ] = bw_RK2( MODEL.u, Nodes, HIST, link_properties );
    case 'RK4'
        [ Rs,Ks, HIST ] = bw_RK4( MODEL.u, Nodes, HIST, link_properties );
end

MODEL.BW.HistBWtemp = HIST;
% MODEL.BW.HistBW = HIST;
MODEL.BW.HistR(:,nt)=Rs; MODEL.BW.HistU(:,nt)=HIST.Um;

Fnl =sparse( [Nodes(:,1); Nodes(:,2)], ones(size(Nodes,1)*2,1), [Rs; -Rs], ndof, 1);
MODEL.fint=MODEL.fint+Fnl;

rows = [ Nodes(:,1); Nodes(:,1); Nodes(:,2); Nodes(:,2) ];
columns = [ Nodes(:,1); Nodes(:,2); Nodes(:,1); Nodes(:,2) ];
values=[ Ks; -Ks; -Ks; Ks];
