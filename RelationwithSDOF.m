...
% with BW links and demonstrate the equivalency of the two provided
...
%Select Model - Model with only horizontal beam links or model with
%additional links on the columns of first floor

% MODEL = InputFileLinksBeams();                %Model with only horizontal beam links
% MODEL = InputFileLinksFirst();                %Additional links on the columns of first floor
...
%% Evaluate model
MODELC = BoucWenRunSDOF(MODEL,Input);

%% Equivalent formulation in vectorized version
%Attention: MODEL.HistR does not maintain the same order
rmpath('coreBWSDOF')
addpath core
